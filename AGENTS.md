...
**Central config funnel**: [src/config/index.ts](src/config/index.ts) re-exports `siteConfig` composed of [site.ts](src/config/site.ts), [navigation.ts](src/config/navigation.ts), [home.ts](src/config/home.ts). All page content (hero, profile, stack, projects, writing, contact, social links) is data-driven from here — edit config, not JSX. `homeConfig` is typed via a local `HomeConfig` interface in [home.ts](src/config/home.ts).

Two shape rules worth knowing before editing it. **Every section object owns its `heading`**, so no page hardcodes copy — which is why `stack` and `projects` are wrappers (`stack.groups`, `projects.items`) rather than bare arrays, and why `writing` exists as a config block even though its posts come from the content collection. The section numerals (`01`…`05`) stay in the markup, since they encode section order rather than content. And **`hero.headline` is a `string[]`**, one entry per rendered line: the page maps it to `<span class="block">`s, so line breaks stay a layout concern and no config value is ever passed through `set:html`.
...
| 01 Profile | One text column — heading and body together in `md:col-span-8`, body capped at `58ch`; only the numeral hangs in the right margin (`md:col-start-10 … md:text-right`).                     |
| 02 Stack   | Colophon grid — `sm:grid-cols-2 lg:grid-cols-3` of hairline-topped blocks, `meta` group label over serif items. Five groups leaves a ragged final row; that is intended.                   |
| 03 Work    | Same two-position idea per entry — index numeral, title and description in `md:col-span-7`; tags and links hang in `md:col-start-10 md:col-span-3`.                                        |
| 04 Writing | Contents page — dates hang right (`md:col-start-10 … md:text-right`) via [PostRow.astro](src/components/PostRow.astro).                                                                    |
| 05 Contact | Statement across the page at `text-headline`, then a split: description left, email + socials right.                                                                                       |

Profile and Work deliberately share one geometry — a single text column with metadata hanging in the right margin — because a three-position split (heading left, numeral far right, body somewhere between) read as scattered. They stay distinct through content structure: one is a single prose block, the other a numbered list of entries. Everything else on the page uses a different shape. Never route body copy into a column narrower than ~5 of 12; that is what made the old Work section a thin tall stack beside a short title.
...
**Type roles** — serif is display only (hero, section headings, project/post titles, the wordmark); Inter carries lede, body, buttons, and nav; JetBrains Mono is reserved for metadata (section numerals, dates, tags, group labels, footer) and code.
