...
**Design language — editorial**: large serif display type, generous whitespace, an asymmetric 12-col grid, hairline rules, and numerals as ornament. Page sections are full-bleed with a `border-t border-line` hairline; the inner `mx-auto max-w-6xl px-6 md:px-10 lg:px-16` grid does the constraining (`<main>` itself is unstyled). Asymmetry comes from varying `md:col-start-*` per section, not from repeating one rail. There is no filled sidebar, no card chrome, no shadows, and no border radii outside code blocks.
...
Do not reintroduce GSAP or any JS-driven reveal. The only JS on the site is the theme init/toggle, the code-block copy button, and ~4 lines that dismiss the popover menu on same-page anchor clicks.
...
**Type roles** — serif is display only (hero, section headlines, project/post titles, the wordmark); Inter carries lede, body, buttons, and nav; JetBrains Mono is reserved for metadata (section numerals, dates, tags, footer) and code, always as `font-mono text-xs tracking-[0.14em] uppercase`. Two fluid display sizes are `@theme` tokens rather than arbitrary values — `--text-display` and `--text-headline`, each with paired `--text-*--line-height` / `--text-*--letter-spacing`, which Tailwind emits as the `text-display` / `text-headline` utilities.
...
- The **mobile menu is a native popover**, not a JS-driven overlay. `<button popovertarget="mobile-menu">` gets open/close, Escape, light-dismiss, and implicit `aria-expanded` from the platform. Everything visual lives in [global.css](src/styles/global.css): the popover's UA styles are overridden to full-bleed, entry/exit use `@starting-style` + `transition-behavior: allow-discrete`, scroll is locked via `html:has(#mobile-menu:popover-open)`, and the burger→X morph is `:root:has(#mobile-menu:popover-open) #line-1/#line-2`. Because classes (not inline styles) own the burger color, switching theme while the menu is open can't strand a stale value — don't reintroduce JS that writes inline styles here. The one thing the platform doesn't do is dismiss on same-page anchor clicks, which is what the small `<script>` at the bottom of [Layout.astro](src/layouts/Layout.astro) handles.
